...
For the machine learning models, I created bagging, random forest, and XGBoost models using `randomForest` from randomForest and `train` from caret. For bagging, I set the 'mtry' parameter in `randomForest` to 5, which is the number of predictors for each outcome, while for random forest, I used the default values. For the XGBoost models, I performed grid search on various hyperparameters in `train`, such as 1, 3, 5, and 7 for 'max_depth' and 0.001, 0.01, 0.1, and 0.2 for 'eta'.
...
imp7 |>
  ...
  coord_flip() +
  plot_layout(ncol = 3) +
  plot_annotation(title = "Variable importance for predicting the variable 'hem' using:")
```
...
imp8 |>
  ...
  coord_flip() +
  plot_layout(ncol = 3) +
  plot_annotation(title = "Variable importance for predicting the variable 'clot' using:")
```
...
imp9 |>
  ...
  coord_flip() +
  plot_layout(ncol = 3) +
  plot_annotation(title = "Variable importance for predicting the variable 'stream' using:")
```
